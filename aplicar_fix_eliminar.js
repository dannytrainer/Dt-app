const fs = require('fs');
const path = process.env.HOME + '/Dt-app/public/js/admin-inicio.js';
const nuevo = fs.readFileSync(process.env.HOME + '/Dt-app/fix_eliminar.js', 'utf8');
let contenido = fs.readFileSync(path, 'utf8');
const inicioMarcador = 'function eliminarEjercicio(idx){';
const finMarcador = 'function togglePresencial(';
...
const resultado = contenido.slice(0, idxInicio) + nuevo + contenido.slice(idxFin);
