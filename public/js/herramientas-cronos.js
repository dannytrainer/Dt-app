...
// ═══════════════════════════════
// PANEL DE COBROS PENDIENTES (reemplaza a la Difusión vieja)
// ═══════════════════════════════
let _cobrosClientes=[];
let _cobrosSeleccionados=new Set();
let _cobrosCola=[];
let _cobrosColaIndex=0;

async function cargarPanelCobros(){
  const eid=(JSON.parse(localStorage.getItem('dt_sesion')||'{}').id)||null;
  const usuarios=await fetch('/api/usuarios?entrenador_id='+eid).then(r=>r.json());
  ...
  const proximos=activos.filter(u=>{
    ...
    if(diff<-3){ d=new Date(hoy.getFullYear(),hoy.getMonth()+1,dia); diff=Math.ceil((d-hoy)/(1000*60*60*24)); }
    return diff>=0&&diff<=3;
  });
  _cobrosClientes=[
    ...vencidos.map(u=>({...u,_estado:'vencido'})),
    ...proximos.map(u=>({...u,_estado:'proximo'}))
  ];
  _cobrosSeleccionados.clear();
  cobrosRenderLista();
}

function _cobrosMensajeDe(u){
  if(u._estado==='vencido'){
    return (u.msg_pago&&u.msg_pago.trim())?u.msg_pago:'El día de hoy se venció tu plan de entrenamiento.';
  }else{
    return (u.msg_proximo&&u.msg_proximo.trim())?u.msg_proximo:('Hola '+u.nombre+' 👋, te recuerdo que tu plan vence pronto. Comunícate conmigo para renovarlo. ¡Gracias! 💪');
  }
}

function cobrosRenderLista(){
  const cont=document.getElementById('cobros-lista');
  if(!cont)return;
  if(_cobrosClientes.length===0){
    cont.innerHTML='<div style="text-align:center;padding:30px 10px;color:#555;font-size:13px"><span style="font-size:28px;display:block;margin-bottom:8px">✅</span>No hay cobros pendientes ni próximos.</div>';
  }else{
    cont.innerHTML=_cobrosClientes.map(u=>{
      const ini=u.nombre?u.nombre.charAt(0).toUpperCase():'?';
      const color=u._estado==='vencido'?'#e31e24':'#ff9800';
      const av=u.foto?'<img src="'+u.foto+'" style="width:44px;height:44px;border-radius:50%;object-fit:cover;border:2px solid '+color+'">':'<div style="width:44px;height:44px;border-radius:50%;background:'+color+'33;border:2px solid '+color+';display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:700;color:'+color+'">'+ini+'</div>';
      const marcado=_cobrosSeleccionados.has(u.id);
      const enviado=!!u.msg_cobro_enviado;
      const estadoTexto=u._estado==='vencido'?'🔴 Cobro vencido':('⚠️ Vence día '+u.dia_pago);
      return '<div style="background:var(--card);border:1px solid #222;border-left:3px solid '+color+';border-radius:14px;padding:12px 14px;display:flex;align-items:center;gap:10px;margin-bottom:8px">'
        +'<div onclick="cobrosToggleSeleccion(\''+u.id+'\')" style="width:22px;height:22px;border-radius:7px;border:2px solid '+(marcado?color:'#333')+';background:'+(marcado?color:'transparent')+';flex-shrink:0;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;font-size:13px;font-weight:900">'+(marcado?'✓':'')+'</div>'
        +av
        +'<div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:700;color:var(--texto)">'+u.nombre+'</div><div style="font-size:10.5px;color:'+color+';font-weight:700;margin-top:1px">'+estadoTexto+'</div>'
        +(enviado?'<div style="font-size:10px;font-weight:800;color:#25d366;background:#0a1f10;border:1px solid #25d366;border-radius:6px;padding:2px 6px;margin-top:3px;width:fit-content">✓ Recordatorio enviado</div>':'')
        +'</div>'
        +'<div onclick="cobrosEnviarIndividual(\''+u.id+'\')" style="background:'+(enviado?'#111':color)+';border:'+(enviado?'1px solid #25d366':'none')+';border-radius:9px;color:'+(enviado?'#25d366':'#fff')+';font-size:11px;font-weight:800;padding:9px 11px;cursor:pointer;white-space:nowrap;flex-shrink:0">'+(enviado?'📲 Reenviar':'📲 WA')+'</div>'
        +'</div>';
    }).join('');
  }
  document.getElementById('cobros-cnt-todos').textContent='('+_cobrosClientes.length+')';
  document.getElementById('cobros-cnt-sinenviar').textContent='('+_cobrosClientes.filter(u=>!u.msg_cobro_enviado).length+')';
  cobrosActualizarBarra();
}

function cobrosToggleSeleccion(id){
  if(_cobrosSeleccionados.has(id))_cobrosSeleccionados.delete(id);
  else _cobrosSeleccionados.add(id);
  cobrosRenderLista();
}

function cobrosSeleccionarTodos(soloSinEnviar){
  const base=soloSinEnviar?_cobrosClientes.filter(u=>!u.msg_cobro_enviado):_cobrosClientes;
  _cobrosSeleccionados=new Set(base.map(u=>u.id));
  cobrosRenderLista();
}

function cobrosLimpiarSeleccion(){
  _cobrosSeleccionados.clear();
  cobrosRenderLista();
}

function cobrosActualizarBarra(){
  const barra=document.getElementById('cobros-barra-sel');
  const aviso=document.getElementById('cobros-aviso-cadena');
  document.getElementById('cobros-contador-sel').textContent=_cobrosSeleccionados.size;
  barra.style.display=_cobrosSeleccionados.size>0?'block':'none';
  if(_cobrosSeleccionados.size>=12){
    aviso.textContent='⚠️ Son '+_cobrosSeleccionados.size+' envíos. Podés cortar en cualquier momento y retomar después.';
...
function cobrosEnviarIndividual(id){
  _cobrosCola=[id];
  _cobrosColaIndex=0;
  cobrosMostrarOverlayPara(id);
}

function cobrosIniciarCadena(){
  if(_cobrosSeleccionados.size===0)return;
  _cobrosCola=Array.from(_cobrosSeleccionados);
  _cobrosColaIndex=0;
  cobrosMostrarOverlayPara(_cobrosCola[0]);
}

function cobrosMostrarOverlayPara(id){
  const u=_cobrosClientes.find(x=>x.id===id);
  if(!u)return;
  document.getElementById('cobros-cadena-avatar').src=u.foto||'';
  document.getElementById('cobros-cadena-nombre').textContent=u.nombre;
  document.getElementById('cobros-cadena-mensaje').textContent=_cobrosMensajeDe(u);
  document.getElementById('cobros-cadena-progreso').textContent=(_cobrosColaIndex+1)+' de '+_cobrosCola.length+' clientes';
  document.getElementById('cobros-cadena-barra').style.width=((_cobrosColaIndex+1)/_cobrosCola.length*100)+'%';
  document.getElementById('cobros-overlay').style.display='flex';
  document.body.style.overflow='hidden';
}

async function cobrosMarcarEnviado(id){
  const u=_cobrosClientes.find(x=>x.id===id);
  if(u)u.msg_cobro_enviado=true;
  try{ await fetch('/api/usuarios/'+id+'/marcar-enviado',{method:'POST'}); }catch(e){}
}

function cobrosAvanzarCadena(){
  _cobrosColaIndex++;
  if(_cobrosColaIndex<_cobrosCola.length){
    cobrosMostrarOverlayPara(_cobrosCola[_cobrosColaIndex]);
  }else{
...
async function cobrosConfirmarEnvio(){
  const id=_cobrosCola[_cobrosColaIndex];
  const u=_cobrosClientes.find(x=>x.id===id);
  if(u){
    const tel=(u.telefono||'').replace(/[^0-9]/g,'');
    const msg=encodeURIComponent(_cobrosMensajeDe(u));
    window.open('https://wa.me/'+tel+'?text='+msg,'_blank');
  }
  await cobrosMarcarEnviado(id);
  cobrosRenderLista();
  cobrosAvanzarCadena();
}

function cobrosOmitirEnvio(){
  cobrosAvanzarCadena();
}
