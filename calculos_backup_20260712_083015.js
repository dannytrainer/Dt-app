...
function calcularProporcionesFemenino(medidas) {
  const { cintura, pecho, cadera, pantorrilla, pierna, brazo, hombros } = medidas;
  const resultado = {};
  ...
  if (cintura && pecho) {
    const ratio = parseFloat((parseFloat(cintura) / parseFloat(pecho)).toFixed(2));
    resultado.cintura_torax = { valor: ratio, ...evaluarZona(ratio) };
  }
  if (cintura && cadera) {
    const ratio = parseFloat((parseFloat(cintura) / parseFloat(cadera)).toFixed(2));
    resultado.cintura_cadera = { valor: ratio, ...evaluarZona(ratio) };
  }
  if (pantorrilla && pierna) {
