...
const idx = c.indexOf("app.post('/api/enviar-rutina/:id'");
const end = c.indexOf("conectarWhatsApp();");
const nuevasRutas = `app.post('/api/enviar-rutina/:id', async (req, res) => {
  ...
  for (const dia of dias) {
    ...
    if (ejs) {
      for (const e of ejs) {
        ...
        texto += '├─────────────────────┤\\n';
        if (e.series) texto += '│ S: ' + e.series.padEnd(17) + ' │\\n';
        if (e.reps) texto += '│ R: ' + e.reps.padEnd(17) + ' │\\n';
        if (e.rir) texto += '│ RIR: ' + e.rir.padEnd(15) + ' │\\n';
        if (e.desc) texto += '│ DESC: ' + e.desc.padEnd(14) + ' │\\n';
        if (e.var) texto += '│ VAR: ' + e.var.padEnd(15) + ' │\\n';
        texto += '└─────────────────────┘\\n\\n';
      }
    } else {
      texto += d.rutina + '\\n\\n';
...
`;
c = c.slice(0, idx) + nuevasRutas + c.slice(end);
